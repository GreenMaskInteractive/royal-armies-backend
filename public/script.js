...
const CHRONICLE_DATA = {
    alpha_0115: {
        ...
        details: "Royal Armies Alpha 0.1.15 expands the playable Age with a living world map, deep settlement workspaces, and richer commander progression.\n\nWhat's new: the world map now carries finalized settlement art and names across multiple nations with color-map and topology views; an experimental map weather system (rain, cloud cover, clear skies, and sunny light on half-hour game ticks) is in development but currently disabled for the time being; an experimental day-night cycle with settlement torch and magic glow is also in development and currently disabled during alpha; move-point travel includes a confirmation step before you spend points; the Blacksmith forge and Armory open full gear shops with category navigation, an inspect panel, an Owned tab with mastery progress, inventory slots, and auto-equip on purchase once you reach the required commander rank; the Barracks workspace matches the settlement modal style with magic unit class portraits and expanded unit rosters; enrolling in the Age plays a full-screen loading theatre on first join and exile re-entry; your nation's capital shows a sweeping shine on the map; Records and roster review modals help you track commander progress; and bordering hostile cities can be assaulted from the world map when diplomacy allows.\n\nDuring alpha, several menu options appear grayed out and cannot be used yet — including Royalty and The Chronicles on the Age Portal (preview tabs only), the Discoveries button in the Nation Hub (song manuscripts, letters, and archives live inside that journal when it opens — no map discoveries can be earned in this build), commander achievements (nothing to unlock or progress here during alpha), nation diplomacy and formal war declarations in Headquarters until elected leadership is established, and settlement buildings below your commander rank (Blacksmith, Armory, Church, and related guild contracts). Experimental map weather and the day-night light cycle remain turned off for now and are not playable in this build.\n\nThank you for helping us shape Royal Armies."
    },
    alpha_0114: {
