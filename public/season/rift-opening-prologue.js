...
(function initRoyalArmiesOpeningPrologue(global) {
    ...
    const TRAILER_CINE_PAN_SCALE_MIN = 1.30;
    const TRAILER_CINE_PAN_SCALE_RANGE = 0.12;
    /** Extra headroom for headless MP4 export so Ken Burns pans never expose letterboxing. */
    const TRAILER_CINE_EXPORT_PAN_SCALE_MIN = 1.40;
    const TRAILER_CINE_EXPORT_PAN_SCALE_RANGE = 0.10;
    /** archimedeslullaby.wav — used when browser metadata is unavailable during export. */
    const TRAILER_ARCHIMEDES_MUSIC_DURATION_SEC = 184.603;
    ...
    let trailerSubtitleImpactSfxGeneration = -1;
    ...
    let trailerMusicDurationSecKnown = null;
    ...
    const TRAILER_MP4_CACHE_BUST = 'trailer-audio-sync-1';
    ...
    /** Reject stale edge-cached exports (~3:06) when the current trailer is ~2:06. */
    const STALE_TRAILER_MP4_MAX_SEC = 145;
    ...
    let trailerMp4Mode = false;
    let trailerMp4VideoEl = null;
    let trailerRenderWatchActive = false;
    let trailerRenderPollTimer = null;
    ...
    function getTrailerMusicDurationSec() {
        if (Number.isFinite(trailerMusicDurationSecKnown) && trailerMusicDurationSecKnown > 0) {
            return trailerMusicDurationSecKnown;
        }
        ...
        return TRAILER_ARCHIMEDES_MUSIC_DURATION_SEC;
    }
    ...
    function clearTrailerFinaleOverlayForScrub() {
    ...
    function shouldStartTrailerFinaleSequenceAtProgress(finaleProgress) {
        return (Number(finaleProgress) || 0) < 0.06;
    }

    function freezeTrailerTimelinePreviewAtCurrentTime() {
        if (!isTrailerReplayMode) return;

        applyTrailerTimelinePosition(trailerReplayTimeSec, {
            syncMusic: false,
            seekMedia: true,
            scrubbing: true,
        });
    ...
    function setTrailerOutroInstant(visible, taglinesVisible) {
        applyTrailerOutroScrubState({
            visible: Boolean(visible),
            taglinesProgress: taglinesVisible ? 1 : 0,
        });
    }

    function clearTrailerOutroScrubStyles(outroEl) {
        if (!outroEl) return;

        outroEl.querySelectorAll('.game-opening-prologue-trailer-call, .game-opening-prologue-trailer-site').forEach((el) => {
            el.style.removeProperty('transition');
            el.style.removeProperty('opacity');
            el.style.removeProperty('transform');
            el.style.removeProperty('clip-path');
        });
        outroEl.querySelector('.game-opening-prologue-trailer-coming-soon')?.style.removeProperty('animation');
    }

    function applyTrailerTaglineScrubFrame(el, progress) {
        if (!el) return;

        const p = Math.max(0, Math.min(1, Number(progress) || 0));
        const motion = 1 - ((1 - p) ** 2.2);
        el.style.transition = 'none';
        el.style.opacity = String(motion);
        el.style.transform = `translateX(${(-12 * (1 - motion)).toFixed(2)}px)`;
        el.style.clipPath = `inset(0 ${(100 * (1 - motion)).toFixed(2)}% 0 0)`;
    }

    function applyTrailerOutroScrubState(options) {
        const opts = options && typeof options === 'object' ? options : {};
        const creditsEl = overlayEl?.querySelector('#game-opening-prologue-trailer-credits');
        const outroEl = creditsEl?.querySelector('#game-opening-prologue-trailer-outro')
            || overlayEl?.querySelector('#game-opening-prologue-trailer-outro');
        if (!outroEl) return;

        const staticFrame = isTrailerScrubPreview() || trailerUserPausedPlayback;
        const visible = Boolean(opts.visible);
        const taglinesProgress = Math.max(0, Math.min(1, Number(opts.taglinesProgress) || 0));
        const callEl = outroEl.querySelector('.game-opening-prologue-trailer-call');
        const siteEl = outroEl.querySelector('.game-opening-prologue-trailer-site');
        const comingSoonEl = outroEl.querySelector('.game-opening-prologue-trailer-coming-soon');

        if (!visible) {
            outroEl.hidden = true;
            outroEl.classList.remove('is-visible', 'is-taglines-visible');
            clearTrailerOutroScrubStyles(outroEl);
            return;
        }

        outroEl.hidden = false;
        outroEl.classList.add('is-visible');
        outroEl.classList.toggle('is-taglines-visible', taglinesProgress > 0.01);

        if (comingSoonEl && staticFrame) {
            comingSoonEl.style.animation = 'none';
        } else if (comingSoonEl) {
            comingSoonEl.style.removeProperty('animation');
        }

        if (staticFrame) {
            if (taglinesProgress <= 0.001) {
                if (callEl) {
                    callEl.style.transition = 'none';
                    callEl.style.opacity = '0';
                    callEl.style.transform = 'translateX(-12px)';
                    callEl.style.clipPath = 'inset(0 100% 0 0)';
                }
                if (siteEl) {
                    siteEl.style.transition = 'none';
                    siteEl.style.opacity = '0';
                    siteEl.style.transform = 'translateX(-12px)';
                    siteEl.style.clipPath = 'inset(0 100% 0 0)';
                }
            } else {
                const callProgress = Math.min(1, taglinesProgress / 0.81);
                const siteProgress = Math.min(1, Math.max(0, (taglinesProgress - 0.19) / 0.81));
                applyTrailerTaglineScrubFrame(callEl, callProgress);
                applyTrailerTaglineScrubFrame(siteEl, siteProgress);
            }
            return;
        }
    ...
    function shouldApplyTrailerStaticVisuals(options) {
        ...
        return !trailerFinaleSequenceStarted && !trailerCreditsRunning;
    }

    function shouldAutoStartTrailerFinaleSequence() {
        return isTrailerReplayPlaying
            && !trailerSeekActive
            && !trailerUserPausedPlayback
            && !trailerTimelinePreviewLocked
    ...
    function applyTrailerCreditsScrubState(timeSec) {
        ...
        const loreToolEl = overlayEl.querySelector('.game-opening-prologue-lore-tool');
        const creditsEl = overlayEl.querySelector('#game-opening-prologue-trailer-credits');
        const outroPanel = creditsEl?.querySelector('.game-opening-prologue-trailer-credits-panel--outro');
        ...
        const taglineRevealSpanMs = 520;

        if (elapsedMs < cursorMs + timings.outroInMs) {
            const phaseMs = elapsedMs - cursorMs;
            const fadeT = Math.min(1, phaseMs / timings.outroFadeInMs);
            const taglinesProgress = phaseMs <= timings.outroTaglinesMs
                ? 0
                : Math.min(1, (phaseMs - timings.outroTaglinesMs) / taglineRevealSpanMs);
            setPanelOpacity(outroPanel, fadeT);
            applyTrailerOutroScrubState({ visible: fadeT > 0.01, taglinesProgress });
            return;
        }

        cursorMs += timings.outroInMs;

        if (elapsedMs < cursorMs + timings.outroHoldMs) {
            setPanelOpacity(outroPanel, 1);
            applyTrailerOutroScrubState({ visible: true, taglinesProgress: 1 });
            return;
        }

        cursorMs += timings.outroHoldMs;

        if (elapsedMs < cursorMs + timings.outroOutMs) {
            const t = (elapsedMs - cursorMs) / timings.outroOutMs;
            setPanelOpacity(outroPanel, 1 - t);
            applyTrailerOutroScrubState({ visible: t < 0.98, taglinesProgress: 1 });
            return;
        }

        cursorMs += timings.outroOutMs;
        setPanelOpacity(outroPanel, 0);
        applyTrailerOutroScrubState({ visible: false, taglinesProgress: 0 });
    ...
    function getTrailerTimelineDurationSec() {
        if (trailerMp4Mode && trailerMp4VideoEl) {
            const mp4Duration = Number(trailerMp4VideoEl.duration);
    ...
    function scheduleTrailerFullscreenRestore() {
        if (!trailerWantsFullscreen || isTrailerFullscreenActive()) return;

        global.requestAnimationFrame(() => {
            restoreTrailerFullscreenIfNeeded();
        });
    }

    function restoreTrailerFullscreenIfNeeded() {
        if (!trailerWantsFullscreen || isTrailerFullscreenActive()) return;
    ...
    function resolveTrailerApiUrl(apiPath) {
        if (typeof global.resolveRoyalArmiesApiUrl === 'function') {
            return global.resolveRoyalArmiesApiUrl(apiPath);
        }
        return apiPath;
    }

    function buildTrailerMp4Src(cacheToken) {
        const token = String(cacheToken || TRAILER_MP4_CACHE_BUST).trim() || TRAILER_MP4_CACHE_BUST;
        return `${TRAILER_MP4_SRC}?v=${encodeURIComponent(token)}`;
    }

    async function fetchTrailerRenderStatus() {
        try {
            const response = await global.fetch(resolveTrailerApiUrl(TRAILER_RENDER_STATUS_PATH), {
                cache: 'no-store',
    ...
    function pauseTrailerCompositionMedia() {
        isTrailerReplayPlaying = false;
        trailerUserPausedPlayback = true;
        audioEl?.pause();
        pauseTrailerMusicPlayback();
        stopTrailerReplaySyncLoop();
        if (trailerFinaleSequenceStarted || trailerCreditsRunning) {
            suspendTrailerFinaleSequencesForScrub();
        }
    }

    function silenceTrailerCompositionAudioForMp4() {
        audioEl?.pause();
        if (audioEl) {
            audioEl.muted = true;
            audioEl.volume = 0;
    ...
    function restoreTrailerCompositionAudio() {
        if (trailerMp4Mode || isTrailerRenderMode()) return;
    ...
    function resumeTrailerMp4IfInterrupted() {
        if (!trailerMp4Mode || !trailerMp4VideoEl || !shouldKeepTrailerMediaPlaying()) return;

        if (trailerMp4VideoEl.paused && !trailerMp4VideoEl.ended) {
            trailerMp4VideoEl.play().catch(() => {});
        }
    }

    function ensureTrailerRenderProgressOverlay() {
        const viewportEl = overlayEl?.querySelector('#game-opening-prologue-trailer-viewport');
        if (!viewportEl) return null;

        let progressEl = viewportEl.querySelector('#game-opening-prologue-trailer-render-progress');
        if (progressEl) return progressEl;

        progressEl = global.document.createElement('div');
        progressEl.id = 'game-opening-prologue-trailer-render-progress';
        progressEl.className = 'game-opening-prologue-trailer-render-progress';
        progressEl.innerHTML = `
            <div class="game-opening-prologue-trailer-render-progress-inner">
                <p class="game-opening-prologue-trailer-render-progress-label">Rendering trailer video</p>
                <p class="game-opening-prologue-trailer-render-progress-percent" id="game-opening-prologue-trailer-render-progress-percent">0%</p>
                <p class="game-opening-prologue-trailer-render-progress-detail" id="game-opening-prologue-trailer-render-progress-detail">Starting…</p>
            </div>
        `.trim();
        viewportEl.appendChild(progressEl);
        return progressEl;
    }

    function updateTrailerRenderProgressOverlay(status) {
        ensureTrailerRenderProgressOverlay();

        const percent = Math.max(0, Math.min(100, Number(status?.percent) || 0));
        const percentEl = overlayEl?.querySelector('#game-opening-prologue-trailer-render-progress-percent');
        const detailEl = overlayEl?.querySelector('#game-opening-prologue-trailer-render-progress-detail');
        const labelEl = overlayEl?.querySelector('.game-opening-prologue-trailer-render-progress-label');

        if (percentEl) {
            percentEl.textContent = `${percent.toFixed(1)}%`;
        }
        if (detailEl) {
            detailEl.textContent = String(status?.message || status?.phase || 'Rendering…');
        }
        if (labelEl) {
            if (status?.status === 'error') {
                labelEl.textContent = 'Trailer render failed';
            } else if (status?.status === 'complete') {
                labelEl.textContent = 'Trailer video ready';
            } else {
                labelEl.textContent = 'Rendering trailer video';
            }
        }
    }

    function stopTrailerRenderProgressWatch() {
        trailerRenderWatchActive = false;
        if (trailerRenderPollTimer) {
            global.clearInterval(trailerRenderPollTimer);
            trailerRenderPollTimer = null;
        }
    }

    async function handleTrailerRenderStatusUpdate(status) {
        if (!status) return;
        ...
        if (status.status === 'complete') {
            if (status.mp4Available) {
                ...
                const cacheToken = TRAILER_MP4_CACHE_BUST;
    ...
    function beginTrailerRenderProgressWatch(initialStatus) {
        trailerRenderWatchActive = true;
        overlayEl?.classList.add('is-trailer-render-progress', 'is-trailer-composition-paused');
        pauseTrailerCompositionMedia();
        updateTrailerRenderProgressOverlay(initialStatus || { percent: 0, message: 'Starting…' });
        ensureTrailerRenderProgressOverlay();

        stopTrailerRenderProgressWatch();
        trailerRenderWatchActive = true;
        trailerRenderPollTimer = global.setInterval(() => {
            void fetchTrailerRenderStatus().then(handleTrailerRenderStatusUpdate);
        }, TRAILER_RENDER_POLL_MS);

        void handleTrailerRenderStatusUpdate(initialStatus);
    }

    function ensureTrailerMp4Video(cacheToken) {
        const viewportEl = overlayEl?.querySelector('#game-opening-prologue-trailer-viewport');
        if (!viewportEl) return null;

        if (!trailerMp4VideoEl) {
            trailerMp4VideoEl = global.document.createElement('video');
            trailerMp4VideoEl.id = 'game-opening-prologue-trailer-mp4-video';
            trailerMp4VideoEl.className = 'game-opening-prologue-trailer-mp4-video';
            trailerMp4VideoEl.preload = 'auto';
            trailerMp4VideoEl.playsInline = true;
            trailerMp4VideoEl.setAttribute('playsinline', '');
            trailerMp4VideoEl.controls = false;
            viewportEl.insertBefore(trailerMp4VideoEl, viewportEl.firstChild);
        }

        trailerMp4VideoEl.src = buildTrailerMp4Src(cacheToken);
        ensureTrailerMp4PauseGuard();
        return trailerMp4VideoEl;
    }

    function waitForTrailerMp4Metadata(videoEl) {
        ...
        const timeoutMs = isTrailerMobileDevice() ? 22000 : 12000;

        return new Promise((resolve) => {
            const finish = (ok) => resolve(ok);
            const onReady = () => finish(Number.isFinite(videoEl.duration) && videoEl.duration > 0);
            videoEl.addEventListener('loadedmetadata', onReady, { once: true });
            videoEl.addEventListener('error', () => finish(false), { once: true });
            videoEl.load();
            global.setTimeout(() => finish(Number.isFinite(videoEl.duration) && videoEl.duration > 0), timeoutMs);
        });
    }

    async function activateTrailerMp4Mode(cacheToken) {
        if (!overlayEl) return false;

        let srcToken = String(cacheToken || TRAILER_MP4_CACHE_BUST).trim() || TRAILER_MP4_CACHE_BUST;
        let videoEl = ensureTrailerMp4Video(srcToken);
        let ready = await waitForTrailerMp4Metadata(videoEl);
        if (!ready) {
            trailerMp4Mode = false;
            return false;
        }

        if (Number.isFinite(videoEl.duration) && videoEl.duration > STALE_TRAILER_MP4_MAX_SEC) {
            srcToken = `${TRAILER_MP4_CACHE_BUST}-${Date.now()}`;
            videoEl.src = buildTrailerMp4Src(srcToken);
            ready = await waitForTrailerMp4Metadata(videoEl);
            if (!ready) {
                trailerMp4Mode = false;
                return false;
            }
        ...
        silenceTrailerCompositionAudioForMp4();
        return true;
    }

    async function probeTrailerMp4Available(cacheToken) {
        ensureOverlay();
        mountTrailerPlayerLayout();
        const videoEl = ensureTrailerMp4Video(cacheToken);
        return waitForTrailerMp4Metadata(videoEl);
    }

    function mountTrailerPlayerLayout() {
        if (!overlayEl) return;

        const wasFullscreen = isTrailerFullscreenActive();
        ...
        trailerPlayerLayoutMounted = true;

        if (wasFullscreen && (domMutated || !isTrailerFullscreenActive())) {
            scheduleTrailerFullscreenRestore();
        }
    }
    ...
    async function beginTrailerFinaleSequence() {
        ...
        if (trailerSeekActive || trailerUserPausedPlayback) {
    ...
    function syncTrailerPlaybackClock() {
        if (trailerMp4Mode && trailerMp4VideoEl) {
            trailerReplayTimeSec = Number(trailerMp4VideoEl.currentTime) || 0;
            return;
        }
    ...
    function resetTrailerFinaleAudioCues() {
    ...
    function syncTrailerFinaleAudioCues(timeSec) {
        ...
        const finaleStart = getTrailerFinaleStartSec();
        const finaleEnd = getTrailerFinaleEndSec();
        const clamped = Number(timeSec) || 0;
        if (clamped < finaleStart || clamped >= finaleEnd) return;

        const generation = logoRevealGeneration;
        ...
        trailerSubtitleImpactSfxGeneration = generation;
    ...
    function resumeTrailerTimelineMedia(timeSec) {
        ...
        if (timeSec < finaleStart - 0.03) {
            overlayEl?.classList.remove('is-trailer-finale-visible');
            setSubtitleDockActive(shouldShowTrailerNarrationSubtitleDock());
            unlockTrailerImpactAudio();
    ...
    function applyTrailerTimelinePosition(timeSec, options) {
        ...
        const seekMedia = opts.seekMedia !== false;
        const scrubbing = Boolean(opts.scrubbing) || isTrailerScrubPreview();
        const staticVisuals = shouldApplyTrailerStaticVisuals({ seekMedia, scrubbing });
        const narrationSec = getTrailerNarrationDurationSec();
        const finaleStart = getTrailerFinaleStartSec();
        const finaleEnd = getTrailerFinaleEndSec();
        ...
        if (trailerMp4Mode && trailerMp4VideoEl) {
            if (seekMedia || scrubbing) {
                trailerMp4VideoEl.currentTime = clamped;
            }
            updateTrailerPlayerUi();
            return;
        }
        ...
        if (clamped < finaleStart - 0.03) {
            if (scrubbing && seekMedia) {
                clearTrailerFinaleOverlayForScrub();
            } else if (trailerFinaleSequenceStarted || isPostNarrationHold || isTrailerFinaleLocked) {
            ...
            } else if (shouldAutoStartTrailerFinaleSequence()) {
                if (shouldStartTrailerFinaleSequenceAtProgress(finaleProgress)) {
            ...
            if (staticVisuals) {
                ...
                if (shouldStartTrailerFinaleSequenceAtProgress(
                    (clamped - finaleStart) / Math.max(0.001, finaleEnd - finaleStart)
                )) {
                    void beginTrailerFinaleSequence();
                } else {
                    overlayEl.classList.remove('is-cinematics-active');
    ...
    function handleTrailerNarrationEnded() {
        ...
        applyTrailerTimelinePosition(trailerReplayTimeSec, { syncMusic: false, seekMedia: true });

        const finaleProgress = (trailerReplayTimeSec - finaleStart)
            / Math.max(0.001, getTrailerFinaleEndSec() - finaleStart);
        if (shouldAutoStartTrailerFinaleSequence()
            && shouldStartTrailerFinaleSequenceAtProgress(finaleProgress)) {
            void beginTrailerFinaleSequence();
        }
    ...
    function ensureTrailerMp4PauseGuard() {
        bindTrailerMediaPauseGuard(trailerMp4VideoEl);
    }
    ...
    function bindTrailerVisibilityKeepalive() {
        ...
        global.document.addEventListener('visibilitychange', () => {
            ...
            if (global.document.visibilityState === 'hidden') {
                if (isTrailerReplayPlaying && (trailerFinaleSequenceStarted || trailerCreditsRunning)) {
                    suspendTrailerFinaleSequencesForScrub();
                }
            ...
            applyTrailerTimelinePosition(trailerReplayTimeSec, {
                syncMusic: false,
                seekMedia: true,
                scrubbing: isTrailerScrubPreview() || trailerUserPausedPlayback,
            });
            resumeTrailerMediaIfInterrupted();
            if (!trailerReplayFrame && !trailerReplayInterval && isTrailerReplayPlaying) {
                startTrailerReplaySyncLoop();
        ...
        global.addEventListener('pageshow', (event) => {
            if (!event.persisted || !isTrailerReplayMode) return;
    ...
    function finishTrailerSeek() {
        ...
        trailerReplayTimeSec = readTrailerSeekSliderTimeSec();
    ...
    function pauseTrailerReplayPlayback() {
        ...
        if (trailerFinaleSequenceStarted || trailerCreditsRunning || trailerReplayTimeSec >= getTrailerFinaleStartSec() - 0.03) {
            suspendTrailerFinaleSequencesForScrub();
        }
        lockTrailerTimelinePreview();
    ...
    async function toggleTrailerFullscreen() {
        ...
        try {
            if (isTrailerFullscreenActive()) {
                ...
                trailerWantsFullscreen = true;
                if (isTrailerMobileDevice()) {
                    await requestTrailerLandscapeLock();
                }
            }
        } catch (_err) {
            trailerWantsFullscreen = isTrailerFullscreenActive();
        }
    ...
    function restartTrailerReplay() {
        ...
        trailerWantsFullscreen = false;
        trailerPlayerLayoutMounted = false;
        isPostNarrationHold = false;
    ...
    function bindTrailerPlayerControls() {
        ...
        if (controlsEl.parentElement !== viewportEl) {
            viewportEl.appendChild(controlsEl);
        }
        trailerControlsViewportEl = viewportEl;
    ...
    function enterTrailerReplayMode(modeOptions) {
        if (!isTrailerPage() || !overlayEl || isTrailerReplayMode) return;

        const opts = modeOptions && typeof modeOptions === 'object' ? modeOptions : {};
        const useComposition = opts.useComposition !== false;
        const useAutoplay = opts.useAutoplay !== false;
        ...
        if (useComposition) {
            ...
            pauseTrailerCompositionMedia();
            overlayEl.classList.add('is-trailer-composition-paused');
        }
        ...
        if (!isTrailerRenderMode() && useAutoplay && !trailerRenderWatchActive) {
    ...
    function ensurePrologueCriticalStyles() {
        ...
        style.textContent = `
            ...
            #${OVERLAY_ID}.is-trailer-render-mode .game-opening-prologue-subtitle-dock {
                opacity: 0 !important;
    ...
    async function waitForTrailerMusicMetadata() {
        ...
        if (Number.isFinite(musicAudio.duration) && musicAudio.duration > 0) {
            trailerMusicDurationSecKnown = musicAudio.duration;
            return;
        }
        ...
        const resolvedDuration = Number(musicAudio.duration);
        trailerMusicDurationSecKnown = Number.isFinite(resolvedDuration) && resolvedDuration > 0
            ? resolvedDuration
            : TRAILER_ARCHIMEDES_MUSIC_DURATION_SEC;
    }
    ...
    function ensureTrailerCinematicPanParams(shotEl) {
        ...
        const panScale = resolveTrailerCinematicPanScaleRange();
        const scale = isTrailerReplayMode || isTrailerRenderMode()
            ? panScale.min + (seededUnitRandom(shotId * 11.13) * panScale.range)
    ...
    function assignRandomCinematicPan(imgEl, durationSec) {
        ...
        const panScale = resolveTrailerCinematicPanScaleRange();
        const scale = isTrailerReplayMode || isTrailerRenderMode()
            ? panScale.min + (Math.random() * panScale.range)
    ...
    function shouldShowTrailerNarrationSubtitleDock() {
        return !isTrailerRenderMode()
            && !trailerMp4Mode
            && areTrailerNarrationSubtitlesEnabled();
    }

    function hideTrailerSubtitleDockForExport() {
        if (!overlayEl) return;
    ...
    function resolveTrailerCinematicPanScaleRange() {
        if (isTrailerRenderMode()) {
            return {
                min: TRAILER_CINE_EXPORT_PAN_SCALE_MIN,
                range: TRAILER_CINE_EXPORT_PAN_SCALE_RANGE,
            };
        }

        if (isTrailerReplayMode) {
            return {
                min: TRAILER_CINE_PAN_SCALE_MIN,
                range: TRAILER_CINE_PAN_SCALE_RANGE,
            };
        }
    ...
    function ensureOverlay() {
        ...
        if (overlayEl && global.document.contains(overlayEl)) {
            ...
            const missingTrailerOutroPanel = isTrailerPage()
                && !overlayEl.querySelector('.game-opening-prologue-trailer-credits-panel--outro');
        ...
        overlayEl.innerHTML = `
            ...
            <div class="game-opening-prologue-logo-stage" aria-hidden="true">
                ...
                <div
                    ...
                    aria-live="polite"
                >
                    <div class="game-opening-prologue-trailer-credits-panel game-opening-prologue-trailer-credits-panel--outro">
                        <div
    ...
    function waitTrailerPlaybackMs(ms, generation) {
        ...
        return new Promise((resolve) => {
            const tick = () => {
                ...
                if (!isTrailerReplayPlaying || trailerSeekActive || trailerUserPausedPlayback) {
                    resolve(false);
                    return;
                }
    ...
    async function runTrailerCreditsSequence(generation) {
        ...
        const alphaPanel = creditsEl?.querySelector('.game-opening-prologue-trailer-credits-panel--alpha');
        const thanksPanel = creditsEl?.querySelector('.game-opening-prologue-trailer-credits-panel--thanks');
        if (!creditsEl || !outroPanel || !outroEl || !greenmaskPanel || !alphaPanel || !thanksPanel) {
            trailerCreditsRunning = false;
            return;
        }
        ...
        const outroFadeInPromise = animateTrailerElementsOpacity([outroPanel], 0, 1, timings.outroFadeInMs);

        if (!(await waitTrailerPlaybackMs(timings.outroTaglinesMs, generation))) {
            trailerCreditsRunning = false;
            return;
        }

        outroEl.classList.add('is-taglines-visible');
        await outroFadeInPromise;
        ...
        if (!(await waitTrailerPlaybackMs(timings.outroHoldMs, generation))) {
            trailerCreditsRunning = false;
            return;
        }
    ...
    function hideTrailerOutro() {
        ...
        const outroPanel = overlayEl?.querySelector('.game-opening-prologue-trailer-credits-panel--outro');
        if (!outroEl) return;

        outroEl.hidden = true;
        outroEl.classList.remove('is-visible', 'is-taglines-visible');
        if (outroPanel) {
            outroPanel.style.removeProperty('opacity');
        }
    }
    ...
    async function waitForEnterWarGate(generation) {
        ...
        if (isTrailerPage()) {
            ...
            const holdOk = await waitTrailerPlaybackMs(TRAILER_MAIN_FINALE_HOLD_MS, generation);
    ...
    async function startTrailerPlayerExperience(options) {
        ...
        const renderStatus = await fetchTrailerRenderStatus();
        ...
        const mp4CacheToken = TRAILER_MP4_CACHE_BUST;

        if (renderStatus?.mp4Available === true && await activateTrailerMp4Mode(mp4CacheToken)) {
            enterTrailerReplayMode({ useComposition: false, useAutoplay: true });
            return Promise.resolve('mp4');
        }

        if (await probeTrailerMp4Available(TRAILER_MP4_CACHE_BUST)
            && await activateTrailerMp4Mode(TRAILER_MP4_CACHE_BUST)) {
            enterTrailerReplayMode({ useComposition: false, useAutoplay: true });
            return Promise.resolve('mp4');
        }

        trailerMp4Mode = false;
        overlayEl?.classList.remove('is-trailer-mp4-mode');
        restoreTrailerCompositionAudio();
    ...
    async function startTrailerRenderExport() {
        ...
        if (!Number.isFinite(trailerMusicDurationSecKnown) || trailerMusicDurationSecKnown < 150) {
            trailerMusicDurationSecKnown = TRAILER_ARCHIMEDES_MUSIC_DURATION_SEC;
        }
